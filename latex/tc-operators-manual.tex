...
\begin{center}
    {\LARGE \textbf{TC OPERATOR'S MANUAL}}\\[0.5em]
    {\large Theographic Calculus Reference Guide v1.0}\\[0.75em]
\end{center}
...
\textbf{Purpose.}
This manual is the comprehensive reference guide for TC (Theographic Calculus) operators, syntax, and usage patterns. It is designed for researchers, engineers, and practitioners who need a systematic understanding of TC's operator grammar for describing physical system behaviors.
...
%======================================================================
\section{Introduction to TC}
%======================================================================

\textbf{Theographic Calculus (TC)} is a minimal operator grammar for describing how physical systems change across multiple domains: geometry, wave dynamics, chemistry, and biology.

TC operates on three fundamental principles:
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Predictivity:} TC sentences map to observable physical regimes
\end{enumerate}

An TC sentence has the canonical form:
\[
...
TC describes physical reality through three fundamental fields, called \textbf{spirals}, each representing a distinct aspect of physical systems:
...
TC defines six universal operations that apply across all domains. Each operation has a specific symbol and meaning.
...
\textbf{Symbol:} \verb|m| (lowercase m) or $\mathrm{CLT}$ (CLT = Coherence Lock Threshold)
...
Domains specify which field (spiral) is primarily active in an TC sentence.
...
An TC sentence follows this structure:
...
TC sentences predict specific physical regimes. These are labeled A1 through A8:
...
\subsection{Constructing an TC Sentence}
...
\subsection{Reading an TC Sentence}
...
\subsection{Testing an TC Prediction}

TC sentences are falsifiable hypotheses. To test:
...
TC sentences describe \textit{tendencies} and \textit{biases}, not deterministic outcomes:
\begin{itemize}[leftmargin=2em]
...
TC predictions hold over ranges of parameters:
\begin{itemize}[leftmargin=2em]
...
\begin{itemize}[leftmargin=2em]
    \item \textit{TC Seven Sentences Test Pack v1.0} — Complete testing protocol
    \item Repository: \url{https://github.com/AceTheDactyl/TC}
\end{itemize}
...
\begin{center}
\textit{Theographic Calculus Operator's Manual v1.0}\\
